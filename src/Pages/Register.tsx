...
export function Register() {
  ...
  // 2. Gerar listas para as datas de forma nativa no React (sem usar o useEffect)
  ...
  // 3. Função de submissão ligada ao Backend
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    // Juntamos o Nome e Apelido para enviar como 'username' para o teu backend
    const username = `${firstName} ${lastName}`.trim();

    // Criamos o objeto que o authController.js espera receber
    const payload = {
      username,
      email,
      password,
    ...
    try {
      ...
      if (response.ok) {
        ...
        alert(data.message || 'Algo correu mal no registo.');
      }
    } catch (error) {
      console.error('Erro ao conectar ao backend:', error);
      alert('Não foi possível contactar o servidor. Garante que o backend está ligado!');
    }
  };
