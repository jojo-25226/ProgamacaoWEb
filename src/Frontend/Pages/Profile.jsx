...
function Profile() {
    ...
    async function loadProfile() {
        ...
        try {
            ...
            // O perfil está restrito a amigos.
            if (err.response?.status === 403) {
                alert(
                    err.response?.data?.message ??
                    "Este perfil está visível apenas para amigos."
                );
    ...
    // Altera a visibilidade do perfil
    ...
    // Verifica se o utilizador atual pode ver o perfil
                        ...
                        {editingBio ? (
                            <div className="bio-edit">
                                <textarea value={bio} onChange={(e) => setBio(e.target.value)}
                                          placeholder="Escreve a tua bio..."/>
                                <div className="bio-actions">
                                    <button onClick={handleSaveBio} className="btn-save">Guardar</button>
                                    <button onClick={() => setEditingBio(false)} className="btn-cancel">Cancelar
                                    </button>
                                </div>
                            </div>
                        ) : (
                            <p className="bio-text" onClick={isMe ? () => setEditingBio(true) : undefined}
