...
function PostCard({post, currentUserId, onDelete}) {
    ...
    async function handleCommentLike(comment) {
        ...
        // Atualiza imediatamente, para a interface parecer mais rápida.
        ...
        try {
            ...
            // Confirma os valores reais devolvidos pelo backend.
            setComments((prev) =>
                prev.map((c) =>
                    c.id === comment.id
                        ? {
            ...
            // Se algo falhar, volta ao estado anterior.
            setComments((prev) =>
                prev.map((c) =>
                    c.id === comment.id
                        ? {
                        ...
                        {comments.map((c) => (
                            <div className="comment" key={c.id}>
                                ...
                                <div className="comment-content">
                                    ...
                                    <button
                                        className={"comment-like-btn" + (c.likedByUser ? " active" : "")}
                                        onClick={() => handleCommentLike(c)}
                                        disabled={commentLikeLoading[c.id]}
                                    >
                                        👍 {c.likedByUser ? "Gostei" : "Gosto"}
